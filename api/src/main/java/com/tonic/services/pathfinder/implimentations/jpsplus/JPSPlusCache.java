...
public class JPSPlusCache
{
    ...
    public JPSPlusCache(int expectedSize) {
     ...
     */
    public List<JPSPlusStep> reconstructPath(int target, int playerStart) {
        LinkedList<JPSPlusStep> path = new LinkedList<>();
        int current = target;

        while (current != -1) {
            Transport transport = transports.get(current);
            path.addFirst(new JPSPlusStep(current, transport));
