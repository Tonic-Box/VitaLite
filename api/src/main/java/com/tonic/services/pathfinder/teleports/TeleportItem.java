...
public enum TeleportItem
{
    ...
    {
        boolean hasQuestReqs = true; //(requirement == null || Quests.getState(requirement) == QuestState.FINISHED);
        switch (this)
        {
            case JATIZSO_LYRE:
            case NEITIZNOT_LYRE:
                return hasQuestReqs && VarAPI.getVar(Varbits.DIARY_FREMENNIK_ELITE) > 0;
            case WATERBIRTH_ISLAND_LYRE:
                return hasQuestReqs && VarAPI.getVar(Varbits.DIARY_FREMENNIK_HARD) > 0;
            case TROLL_STRONGHOLD:
                return false; //hasQuestReqs && Skills.getLevel(Skill.AGILITY) >= 73 && Vars.getBit(Varbits.DIARY_FREMENNIK_HARD) > 0;
