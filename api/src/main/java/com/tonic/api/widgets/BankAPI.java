...
public class BankAPI {
     ...
     * Supports preset quantities (1, 5, 10, all) for compatibility and custom X
     * amounts.
     * 
    ...
    public static boolean withdraw(InventoryLoadout loadout, int maxActionsPerTick) {
        ...
        for (LoadoutItem item : loadout) {
            ...
            // BankAPI#withdraw handles withdraw mode but we need to keep track of actions,
            // so do it ourselves
    ...
    public static void withdraw(String name, int amount, boolean noted) {
        setWithdrawMode(noted);
        ItemEx item = Static.invoke(() -> InventoryQuery.fromInventoryId(InventoryID.BANK).withName(name).first());
    ...
    public static void deposit(int id, int amount) {
        ItemEx item = Static.invoke(() -> InventoryQuery.fromInventoryId(InventoryID.INV).withId(id).first());
    ...
    public static void deposit(String name, int amount) {
        ItemEx item = Static.invoke(() -> InventoryQuery.fromInventoryId(InventoryID.INV).withName(name).first());
    ...
    public static void withdrawAction(int id, int amount, int slot) {
        ...
        WidgetAPI.interact(BankQuantityResolver.withdrawFallbackAction(amount), InterfaceID.Bankmain.ITEMS,
                resolvedSlot, id);
    }
    ...
    public static void depositAction(int id, int amount, int slot) {
        ...
        WidgetAPI.interact(BankQuantityResolver.depositFallbackAction(amount), InterfaceID.Bankside.ITEMS, resolvedSlot,
                id);
    }
    ...
    /**
     * Deposits all items from the containers in inventory into the bank (Empty
     * containers).
     */
    ...
    public static int getSlot(int itemId) {
        ItemEx item = Static.invoke(() -> InventoryQuery.fromInventoryId(InventoryID.BANK).withId(itemId).first());
        if (item == null)
            return -1;
        return item.getSlot();
    }
    ...
    public static void use(String itemName) {
        ItemEx item = Static.invoke(() -> InventoryQuery.fromInventoryId(InventoryID.INV).withName(itemName).first());
        if (item == null)
            return;
    ...
    /**
     * Attempts to guess the slot an item will be withdrawn into (for use in
     * programatically
     * 1-ticking interactions with withdrawn items) and "uses" the item. Not
     * guarenteed to be
     * 100% accurate.
     * 
    ...
    public static void dragItem(int itemId, int fromSlot, int toSlot) {
        ...
        WidgetAPI.dragWidget(InterfaceID.Bankmain.ITEMS, item.getId(), item.getSlot(), InterfaceID.Bankmain.ITEMS,
                itemId2, toSlot);
    }
    ...
    public static void dragItemToTab(int itemId, int fromSlot, int toTab) {
        ...
        WidgetAPI.dragWidget(InterfaceID.Bankmain.ITEMS, item.getId(), item.getSlot(), InterfaceID.Bankmain.TABS, -1,
                tabIndex);
    }
}
