...
import com.tonic.plugins.profiler.sampling.*;
...
public class SamplingTab extends JPanel {
    ...
    // Flame graph
    private com.tonic.plugins.profiler.visualization.FlameGraphPanel flameGraphPanel;
    ...
    private void initializeSamplers() {
        ...
        flameGraphPanel = new com.tonic.plugins.profiler.visualization.FlameGraphPanel();
    }
    ...
    private void buildFlameGraph() {
        List<com.tonic.plugins.profiler.sampling.StackSample> samples = cpuSampler.getStackSamples().getAll();
        if (samples.isEmpty()) {
            return;
        }

        // Create root node
        com.tonic.plugins.profiler.visualization.FlameGraphNode root =
            new com.tonic.plugins.profiler.visualization.FlameGraphNode("(root)", "", "");

        // Build call tree from stack samples
        for (com.tonic.plugins.profiler.sampling.StackSample sample : samples) {
            ...
            com.tonic.plugins.profiler.visualization.FlameGraphNode current = root;
            current.addSample();
